...
The guidance in this page is programming language agnostic.
...
prescriptive formula. The logging framework being used and the
mechanics of output for log streams influence any guidelines, but some broad stokes follow. Consult any specific
documentation on logging for the respective GH repository in [Getting Started](../../index.mdx).
...
There are ways of redacting data in the code, but it is extremely
important given the security of our users to think about and fully understand what is being logging
when writing code. 

:::

When writing code in a logging framework, at a minimum calls consist of a log message and a log level. This level
is compared at runtime and/or compile time to the level filter, which notates the highest level
which will be logged during the program's execution. By default, in production the log level is info.
Generally speaking, debug builds should be able to use the debug level.

When to use each log level is described below in descending order of severity. Note that

...
protect the key with a [strong passphrase or password][password-generation]. Alternatively set up
your [yubikey as your signing key](#hardware-backed-ssh-keys).

```bash
# path to save your private signing key
KEY_FILE=~/.ssh/bw-signing
ssh-keygen -f $KEY_FILE -C "$(git config --global --get user.email)" -t ed25519 test
```
...
1. Configure git to sign using SSH.
   ...
   git config --global user.signingkey "${KEY_FILE}.pub" blah
   ```

2. Follow the [Github documentation][github-verification] to configure commit signing test test

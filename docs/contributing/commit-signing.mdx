...
GitHub supports [commit signing][github-verification] with GPG, SSH and S/MIME.
...
7. (Optional) See Avoiding SSH Key Passphrase Prompts Per Commit below to avoid being prompted for
   your SSH key passphrase every time you sign a commit.
...
#### macOS
    ...
    2. The above command will need to be ran each time you restart your computer.
    If you would like to automate this process, then you can leverage LaunchAgents to run the command on startup.
       ...
       e. You can kickstart the LaunchAgent and confirm it will work by running:
...
recover from the lose of a YubiKey. At least one backup plan should be set up. Some ideas:

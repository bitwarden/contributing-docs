...
Bitwarden also need to support:
...
the stored procedures can be called without the new columns which allows it to run with both the old
and new code.
...
migration_ and _ Finalization migration_ and are described in more details in this section.

#### Initial migration

The initial migration runs before the code deployment, and it's purpose is to add support for
_Release X+1_ without breaking support of _Release X_. This deployment should have execute quickly
and not contain any costly operations to ensure zero downtime.

#### Transition migration

The transition migration are run sometime during the transition phase, and provides an optional data
migration should it be to slow or put to much load on the database, or otherwise making it
...
The finalization migration removes the temporary measurements that was needed to retain backwards

...
To add terminology to compliment the above diagram, migrations that are a part of "Deploy new
changes, migrate data, put in scaffolding code" are considered _Initial_ migrations. Migrations that
are run during the Transition Phase are considered _Transition_ migrations. And the migrations that
run as a part of "Remove old schema, scaffolding code" are considered _Finalization_ migrations. The
definitions of each are helpful when discussing the type of migration in relation to orchestrating

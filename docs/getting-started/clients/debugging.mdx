...
attach the debugger to the Desktop app and the Web Vault without any extra setup. Open the
**Run and Debug** panel and select a configuration from the dropdown.
...
Select **`Desktop: Attach Debugger (main + renderer)",`** from the Run and Debug dropdown. This attaches VSCode's
debugger to a running desktop client. You can then set breakpoints directly in the TypeScript source.

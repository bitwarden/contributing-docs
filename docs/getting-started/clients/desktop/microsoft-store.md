...
# machine.
Export-PfxCertificate -cert "Cert:\CurrentUser\My\$($certificate.Thumbprint)" -FilePath $certPath -Password $password
```
...
```powershell
$env:ELECTRON_BUILDER_SIGN_APPX = "1"
...
cd dist
"C:\Program Files (x86)\Windows Kits\10\bin\10.0.19041.0\x64\signtool.exe" sign /v /fd sha256 /n "CN=14D52771-DE3C-4886-B8BF-825BA7690418" .\Bitwarden-<version>.appx
```
...
pkcs="<file path>.pfx"
pwd_file=$(mktemp)
read -s -p "Set password for certificate: " CERT_PASSWORD
...
openssl pkcs12 -inkey $key -in $cert -export -out $pkcs -passin env:CERT_PASSWORD -passout env:CERT_PASSWORD
# clean up temp files
rm "$key" "$cert"
```

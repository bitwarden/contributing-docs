...
if you encounter some errors, build using the CLI being into the _src/App_ folder (previously
...
:::tip Troubleshooting common mistake
...
You almost surely are trying to build the app from the root folder. Instead go to _src/App_ and try
building again.
...
The Argon2Id library (`libargon2.a`) is loaded using `MTouchExtraArgs` in almost all csproj of the
solution. In order to make this simpler a property was added into **Directory.Build.props** called
`Argon2IdLoadMtouchExtraArgs` which has the code to fill in the extra args parameter. Each csproj is
configured with this property so this is only added on the correct runtime identifiers and we can
...
watchOS app gets in the way. So in order to have a fast way to only care about the main app two
...
Toggling these off can provide a faster developer experience which is really useful on a lot of
...
change that on every csproj so two properties were added to the **Directory.Build.Props** to help
with this:

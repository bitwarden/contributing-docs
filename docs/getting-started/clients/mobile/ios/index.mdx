...
if you encounter some errors build using the CLI (previously removing bin/obj folders): For device:
...
If you find any errors regarding argon2Id library when building for `iossimulator-x64` then try to
build for device `ios-arm64` as there might be some issues regarding arch differences and
requirements for the argon2Id library to be used in `iossimulator-x64`.

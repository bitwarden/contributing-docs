...
"databases:4:connectionString": "server=localhost;port=4306;uid=maria;pwd=_________;database=ef_test;AllowUserVariables=true",
...
tests will connect to. Make sure that you fill in the password for each connection string. It is
recommended that you use a different database for your integration tests as they will produce a lot
of test data.
...
3. Ensure your databases are all migrated (see instructions above). You can use the `-test` flag to
   ...
   ```
   # Migrate integration test databases only
   pwsh migrate.ps1 -test

   # Migrate a specific integration test database
   pwsh migrate.ps1 -postgres -test

   # Migrate a specific development database
   pwsh migrate.ps1 -postgres
   ```

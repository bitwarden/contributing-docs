...
| `content/bootstrap-autofill.ts`                | Initializes the core autofill feature script without the autofill inline menu logic in place.                                                                                                                                                                                                                                                             |
| `content/bootstrap-autofill-inline-menu.ts`    | Initializes the core autofill feature script with the autofill inline menu logic in place.                                                                                                                                                                                                                                                                |
| `content/autofill-init.ts`                     | The core autofill feature script, handles collection of form elements from the page through the `CollectAutofillContentService` class and filling of form fields through the `InsertAutofillContentService` class. Also conditionally handles presentation and behavior of the autofill inline menu through the `AutofillInlineMenuContentService` class. |
...
| `overlay.background.ts`      | Handles communication between the content scripts present on the current page and the inline menu button and list pages that present the autofill inline menu to the user. |

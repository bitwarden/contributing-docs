...
A device has been compromised while the Bitwarden vault is locked (protections are not guaranteed if
device is compromised before the vault is locked). Attacker has full access to userspace, but not
kernel space.

...
You now have the MSSQL server running. The next step is to create the database that will be used by
...
4.  Double-click the `Bitwarden Data Protection Dev` and `Bitwarden Identity Server Dev`
    certificates.
...
1.  Copy the example `secret.json` file.
    ...
    - `secrets.json` is configured to use the dockerized Azurite and MailCatcher instances and is
...
3.  &nbsp;
    ...
    You also need the following additional user secrets:
    ...
    You will also need to download the `Licensing Certificate - Dev` from the shared Engineering

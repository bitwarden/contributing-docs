...
You now have the MSSQL server running in Docker. The next step is to create the database that will be used by
...
1.  Generate the certificates and save them to your keychain:
    ...
    ```
2. You will be prompted to enter a password 3 times. Create a complex password and type it into the `Export Password` field on each prompt. You will not be able to copy/paste.
...
To run your local server environment as a licensed instance, you will need to download the `Licensing Certificate - Dev` from the shared Engineering collection and install it. This can be done by double-clicking on the downloaded certificate.
...
1.  Get a template `secrets.json`. We need to get an initial version of `secrets.json`, which you will modify for your own secrets values.
    ...
    - This `secrets.json` is configured to use the dockerized Azurite and MailCatcher instances and is
...
3.  Once you have your `secrets.json` complete, run this command to add the secrets to each Bitwarden server project:
